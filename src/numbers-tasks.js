...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosAngle = scalarProduct / (lengthVector1 * lengthVector2);
  const calculateAngle = Math.acos(cosAngle);
...
function roundToPowerOfTen(num, pow) {
  const p = 10 ** pow;
  return Number(Math.round(num / p) + '0'.repeat(pow));
}
...
function getCountOfOddNumbers(number) {
  let arr;
  if (number > 0) {
    arr = Array.from({ length: number + 1 }, (item, index) => index);
  }
